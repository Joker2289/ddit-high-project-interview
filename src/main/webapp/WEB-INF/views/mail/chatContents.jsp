								...
								<c:choose>
									<c:when test="${fn:split(chatContentsVo.chat_content,'▣')[1] eq 'image' }">
										<a href="/chatContentsFileDownload?content_code=${chatContentsVo.content_code }">									
											<div style="width: 250px; height: 300px; background-image:url('/chat_contentsImage?uuid=${fn:split(chatContentsVo.chat_content,'▣')[0]}'); background-repeat: no-repeat; background-size: contain; background-position: center;border-radius: 5px;background-color: rgba(0, 117, 174, 0.2);">
											</div>
										</a>
									</c:when>
									<c:otherwise>
										<a href="/chatContentsFileDownload?content_code=${chatContentsVo.content_code }">									
											<div style="max-width: 400px; background-color: rgba(0, 117, 174, 1); border-radius: 5px; font-weight: bold; padding: 5px; color: white;">

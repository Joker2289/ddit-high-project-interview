        ...
        <div class="comment-area ${ref_code }" id="comment-area${comment.comment_code }" data-comment="${comment.comment_code }">
          <div>
            <div class="comment-profile-img">
              <c:choose>
                <c:when test="${comment.mem_division == 1 && comment.profile_path != null }">
                  <c:if test="${comment.mem_id eq SESSION_MEMBERVO.mem_id }">
                  	<a class="contents_writer_image" href="/profileHome">
                  	  <img class="contents_writer_image" src="${ cp }/view/imageView?mem_id=${comment.mem_id }&division=pf">
                  	</a>
                  </c:if>
                  <c:if test="${comment.mem_id != SESSION_MEMBERVO.mem_id }">
                  	<a class="contents_writer_image" href="/profileHome?user_id=${comment.mem_id }">
                  	  <img class="contents_writer_image" src="${ cp }/view/imageView?mem_id=${comment.mem_id }&division=pf">
                  	</a>
                  </c:if>
                </c:when>
                <c:when test="${comment.mem_division == 2 && comment.profile_path != null }">
                  <a class="contents_writer_image" href="/corporation?corp_id=${comment.mem_id }">
                    <img class="contents_writer_image" src="${comment.profile_path }">
                  </a>
                </c:when>
	  	        <c:otherwise>
            ...
            <div style="display: inline-block; width: 75%;">
              <div class="comment-writer" style="display: inline-block; width: 60%;">
                <span style="font-weight: bold;">
                  <c:choose>
                    <c:when test="${comment.mem_division eq '1' }">
                      <c:if test="${comment.mem_id eq SESSION_MEMBERVO.mem_id }">
	                    <a href="/profileHome">${comment.writer_name }</a>
                      </c:if>
                      <c:if test="${comment.mem_id != SESSION_MEMBERVO.mem_id }">
	                    <a href="/profileHome?user_id=${comment.mem_id }">${comment.writer_name }</a>
                      </c:if>
                    </c:when>
                    <c:when test="${comment.mem_division eq '2' }">
                      <a href="/corporation?corp_id=${comment.mem_id }">${comment.writer_name }</a>
                    </c:when>
                  </c:choose>
                </span>
              </div>
              <div class="comment-date-info" style="display: inline-block; width: 40%;">
                <c:choose>

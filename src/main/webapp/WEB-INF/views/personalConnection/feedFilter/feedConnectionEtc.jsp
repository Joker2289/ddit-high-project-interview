...
<c:choose>
	...
	<c:otherwise>
		...
		<c:set var="profile_addrpath" value="/profile?mem_id=${etc.user_id }"/> 
		<c:choose>
			 <c:when test="${fn:contains(etc.profile_path, 'http')}">
		      	<c:set var="profile_path" value="${etc.profile_path }"/> 
		      </c:when>
		</c:choose>
			...
			<div style="width: 225px;">
				<div class="corporation" style="padding-left : 15px;">
					<a href="/profileHome?user_id=${etc.user_id }"><div
						style="width: 85px;height: 85px;background-image:url(${fn:contains(etc.profile_path, 'http') ? profile_path : profile_addrpath});background-repeat: no-repeat;background-size: cover;background-position: center;margin-left: 5px;margin-top: 15px; margin-bottom: 12px;border-radius: 100px;border: 2px solid #E3EEF2;">
					</div></a>
					<a href="/profileHome?user_id=${etc.user_id }"><div>

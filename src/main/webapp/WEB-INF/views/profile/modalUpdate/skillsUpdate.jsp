...
<fmt:formatDate value="${possesion_skillsVo.issue_date }" pattern="yy/MM/dd" var="fmtIssue_date"/>
...
<form action="/possesion_skillsUpdate" method="post" id="possesion_skillsUpdateFrm" enctype="multipart/form-data">
	<div>
		<div class="modalRow">
			...
			<div class="modalHalfRight">
				...
				<input class="form-control" type="text" name="issue_org" value="${possesion_skillsVo.issue_org }">
			</div>
		</div>
		<div class="modalRow">
			<div class="modalHalfLeft">
	...
	<input type="hidden" value="${possesion_skillsVo.pskill_code }" name="pskill_code">
</form>
</div>
...
<script>
	...
	$("#possesion_skillsSave").on("click",function(){
		...
		if($("input[name=issue_date]").val().trim()==""){
 		...
 		$("#possesion_skillsUpdateFrm").submit();
	});
	...
	$("input[name=issue_date]").datepicker('setDate', '${fmtIssue_date}');
</script>
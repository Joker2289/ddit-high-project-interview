...
<ul class="dropdown-menu" style="display: block; top: 425px;width: 155px; min-height: 40px; ">
	...
	<c:if test="${not empty user_id  && SESSION_DETAILVO.user_id != user_id}">
		...
		<c:if test="${empty followVo.mem_id  }">
			<li class="dropdownAccordion otherDropdown" style="width: 155px;">
				<a href="/profileInsertFollow?mem_id=${SESSION_DETAILVO.user_id}&ref_keyword=${user_id }&division=43" style="padding: 0 15px 0 0; width: 140px;"><span style="font-size: 24px;"><i class="fas fa-user-plus"></i></span>팔로우</a>
			</li>
		</c:if>
		<c:if test="${not empty followVo.mem_id  }">
			<li class="dropdownAccordion otherDropdown" style="width: 155px;">
				<a href="/profileDeleteFollow?mem_id=${SESSION_DETAILVO.user_id}&ref_keyword=${user_id }" style="padding: 0 15px 0 0; width: 140px;"><span style="font-size: 24px;"><i class="fas fa-user-minus"></i></span>언팔로우</a>
			</li>
		</c:if>
	</c:if>
